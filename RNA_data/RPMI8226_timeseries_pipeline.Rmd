...
###
################################################################################
#### This pipeline is used to process the log2FC data for the RPMI8226 time series KB528 experiment. 
...
args <- c("","","","",
  WORKSHOPDIR,# 5
  file.path("./RPMI8226_KB528_timeseries_TPM.csv"), # 6
  file.path("KB528_time_series/"), # 7
  "KB528_RPMI8226_analysis",# 8         
  "ignore", # 9
  FALSE)# 10
...
annotation_key <- read.csv("./annotation_RPMI8226.csv")
...
annotation_key$rep <- stringr::str_sub(annotation_key$label,-5,-1)
...
all_fpkm_exprs <- read.delim("./RPMI8226_KB528_timeseries_TPM.txt",sep = "\t",header=TRUE)
print(all_fpkm_exprs[1:5,])
all_fpkm_exprs$FEATURE_ID <- NULL

all_fpkm_exprs <- all_fpkm_exprs[,c("GENE_SYMBOL",annotation_key$label)]
  
colnames(all_fpkm_exprs) <- c("GENE_SYMBOL",paste(annotation_key$group,annotation_key$rep,sep = "_"))

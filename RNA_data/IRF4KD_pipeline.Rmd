---
title: "IRF4_KD_Multiple_Myeloma"
date: "5/1/2023"
...
#### This pipeline is used to process the log2FC data for the IRF4 knockdown experiment. 
...
name = args[8]
groupString = args[9]
erccpath = file.path("~/Workshop_ERCC_norm/ERCC_Controls_Analysis.txt")
...
annotation_key <- read.csv("./annotation_IRF4KD.csv")

annotation_key$group <- paste(annotation_key$condition,annotation_key$cell_line)
...
### pipeline uses "fpkm" for data name, but file is TPM

all_fpkm_exprs <- read.delim("./IRF4KD_TPM.csv",sep = ",")
...
if(useERCC == TRUE){
  ...
  expressedProbesNorm = base::which(apply(all_fpkm_exprs_norm,1,max)>1)
  expressedProbesRaw = base::which(apply(all_fpkm_exprs,1,max)>1)
...
annotation_key$label ### must match colnames here
annotation_key$sample_name <- paste(annotation_key$label,annotation_key$group)
colnames(expressionTable) <- annotation_key$sample_name

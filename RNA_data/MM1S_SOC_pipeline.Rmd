...
#### This pipeline is used to process the log2FC data for the MM1S standard of care (SOC) dataset. 
...
args <- c("","","","",
  WORKSHOPDIR,# 5
  file.path("MM1S_SOC_TPM.txt"),#6
  file.path("MM1S_SOC/"),#7
  "MM1S_SOC_analysis",# 8         
  "ignore", # 9
  FALSE) # 10
...
geneFPKMFile = file.path(basepath,args[6]) ### filename geneFPKMFile, but TPM 
...
annotation_key <- read.csv("./annotation_MM1S_SOC.csv")
annotation_key$label <- paste(annotation_key$label,annotation_key$label,sep = "_")
...
all_fpkm_exprs <- read.delim("./MM1S_SOC_TPM.csv",sep = ",")

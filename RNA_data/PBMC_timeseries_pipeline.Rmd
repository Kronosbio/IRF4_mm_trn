...
### SET YOUR PATH!
### 
#WORKSHOPDIR <- dirname(rstudioapi::getActiveDocumentContext()$path)
WORKSHOPDIR <- "./"

###
###
################################################################################
#### This pipeline is used to process the log2FC data for the PBMC time series KB528 experiment. 
#### Most of the other figures used here are extra QC that are not necessary for the final analysis.
...
args <- c("","","","",
  WORKSHOPDIR,# 5
  file.path("PBMC_KB528_timeseries_TPM.csv"), # 6
  file.path("KB528_time_series/"),# 7
  "PBMC_analysis",# 8         
  "ignore", # 9
  FALSE)# 10
...
# Set up paths
basepath     = file.path(args[5])
geneFPKMFile = file.path(basepath,args[6])
outputFolder = file.path(basepath,args[7])
name = args[8]
#groupString = args[9]
#erccpath = file.path(basepath,"ERCC_Controls_Analysis.txt")
erccpath = file.path("~/p300_RNA/ERCC_Controls_Analysis.txt")
...
annotation_key <- read.csv("./annotation_PBMC.csv")
annotation_key$label <- annotation_key$Sample
annotation_key$group <- paste(annotation_key$Treatment,annotation_key$Time,sep = "_")

# annotation_key$label <- paste(annotation_key$label,"_merged",sep = "")
# annotation_key$group <- paste(annotation_key$condition,annotation_key$cell_line,sep = "_")
groupVector <- unique(annotation_key$group)
...
all_fpkm_exprs <- read.delim("./PBMC_KB528_timeseries_TPM.csv")

colnames(all_fpkm_exprs)[-c(1,2)]

colnames(all_fpkm_exprs) <- stringr::str_replace(colnames(all_fpkm_exprs),"_L002_R1_001.fastq.gz","")

colnames(all_fpkm_exprs)[-c(1,2)] <- substr(colnames(all_fpkm_exprs)[-c(1,2)], 1, nchar(colnames(all_fpkm_exprs)[-c(1,2)]) - 4)

print(all_fpkm_exprs[1:5,])
all_fpkm_exprs$FEATURE_ID <- NULL

all_fpkm_exprs <- all_fpkm_exprs[,c("GENE_SYMBOL",annotation_key$label)]

colnames(all_fpkm_exprs) <- c("GENE_SYMBOL",paste(annotation_key$group,annotation_key$reps,sep = "_"))
#gene row names must be unique.
#this finds all uniquely named rows (takes first instance)
usedGenes = c()
uniqueRows = c()
for(i in 1:nrow(all_fpkm_exprs)){
    geneName = as.character(all_fpkm_exprs[i,1]) ########### EDIT ####
    #geneName = as.character(rownames(all_fpkm_exprs)[i])
...
filename_raw = file.path(outputFolder,paste(name,'_all_fpkm_exprs_raw.txt',sep=''))
...
if(useERCC == TRUE){
  ...
  filename_norm = file.path(outputFolder,paste0(name,'_all_fpkm_exprs_norm.txt'))
...
```{r WITH_ERCC, echo=FALSE}

if(useERCC == TRUE){
  
  
  #plotting spike-ins raw
  filename_spike = file.path(outputFolder,paste(name,'_spike_raw.pdf',sep=''))
  ...
  #plotting spike-ins raw
  filename_spike = file.path(outputFolder,paste(name,'_spike_norm.pdf',sep=''))
  pdf(file=filename_spike,width = 8,height =8)
  ...
  filename_raw = file.path(outputFolder,paste0(name,'_all_fpkm_exprs_raw_scatter.png'))
  
  png(filename=filename_raw, width=png_size, height=png_size,pointsize=24)
    sampleRows = sample(expressedProbesRaw,1000)
    pairs(
      log2(all_fpkm_exprs[sampleRows,]),
      lower.panel=panel.awesome,
      upper.panel=panel.cor,
      cex.labels=0.8,
      xlim=c(axisMinRaw,axisMaxRaw),
      ylim=c(axisMinRaw,axisMaxRaw),
      pch=19,
      col=rgb(0.5,0.5,0.5,0.4),
      cex=1,
      main='Unnormalized log2 expression (fpkm)'
      )
  dev.off()
  
  axisMinNorm = 0
  axisMaxNorm = max(log2(all_fpkm_exprs_norm[expressedProbesNorm,]))
  filename_norm = file.path(outputFolder,paste(name,'_all_fpkm_exprs_norm_scatter.png',sep=''))
  
  png(filename=filename_norm,width =png_size,height =png_size,pointsize=24)
    sampleRows = sample(expressedProbesNorm,1000)
    pairs(
      log2(all_fpkm_exprs_norm[sampleRows,]),
      lower.panel=panel.awesome,
      upper.panel=panel.cor,
      cex.labels=0.8,
      xlim=c(axisMinNorm,axisMaxNorm),
      ylim=c(axisMinRaw,axisMaxRaw),
      pch=19,
      col=rgb(0.5,0.5,0.5,0.4),
      cex=1,
      main='Spike-in normalized log2 expression (fpkm)'
      )
  dev.off()
  
  #now make some boxplots	
  filename_box = file.path(outputFolder,paste(name,'_exprs_boxplot.pdf',sep=''))
...
if(useERCC == FALSE){
  ...
  axisMinRaw = 0
  axisMaxRaw = max(log2(all_fpkm_exprs[expressedProbesRaw,]))
  filename_raw = file.path(outputFolder,paste(name,'_all_fpkm_exprs_raw_scatter.noercc.png',sep=''))
  ...
  #now make some boxplots
  filename_box = file.path(outputFolder,paste(name,'_exprs_boxplot.noercc.pdf',sep=''))
  pdf(file=filename_box,width = 10,height = 8)
...
# KB00284098_annotation_key$label ### must match colnames here
# KB00284098_annotation_key$sample_name <- paste(KB00284098_annotation_key$label,KB00284098_annotation_key$group)
# colnames(expressionTable) <- KB00284098_annotation_key$sample_name
...
#============================REPLICATE ANALYSIS==========================
#========================================================================
```{r REPLICATE_ANALYSIS, echo=FALSE}
#make a multipage pdf with pairiwise scatters for replicates
#put the average correlation in the title

filename_replicates = file.path(outputFolder,paste(name,'_replicate_correlations.pdf',sep=''))
...
filename_means = file.path(outputFolder,paste(name,'_all_fpkm_means.txt',sep=''))
...
filename_means = file.path(outputFolder,paste(name,'_exprs_fpkm_means.txt',sep=''))
write.table(meanMatrix,file=filename_means,quote=FALSE,sep='\t')

```

#========================================================================
#=================CROSS COMPARISONS BETWEEN GROUPS=======================
#========================================================================
```{r CROSSCOMPARISONS, echo=FALSE}
...
#3. pairwise amplifier plots, genes ranked by expression in A with expression in B plotted and vice versa
...
for(i in 1:length(groupVector)){
  ...
  while(j <= length(groupVector)){			
    ...
    filename_pair = file.path(outputFolder,paste(name,'_',group1,'_vs_',group2,'.pdf',sep=''))
    ...
    filename_exp= file.path(outputFolder,paste(name,'_',group1,'_vs_',group2,'_exprs_matrix.txt',sep=''))
    write.table(expMatrix,file=filename_exp,quote=FALSE,sep='\t')
    
    
    #making the gct
    filename_gct= file.path(outputFolder,paste(name,'_',group1,'_vs_',group2,'.gct',sep=''))
    ...
    #making the cls
    filename_cls= file.path(outputFolder,paste(name,'_',group1,'_vs_',group2,'.cls',sep=''))
